...
FROM
    actor
WHERE
    (first_name = 'Joe' OR last_name = 'Joe');
...
SELECT 
    last_name, COUNT(*) AS num_of_actors
FROM
    actor
GROUP BY last_name;
...
SELECT 
    ...
    staff.first_name,
    staff.last_name,
    SUM(payment.amount) AS total_rung_up
FROM
    payment
        JOIN
...
SELECT 
    film.title, COUNT(inventory.inventory_id) AS num_of_copies
FROM
    film
        INNER JOIN
...
FROM
    ...
    WHERE
        country.country = 'Canada') AS city) ON city.city_id = address.city_id) AS address) ON address.address_id = customer.address_id;
...
FROM
    ...
    FROM
        category
    WHERE
        name = 'Family') AS cat) ON cat.category_id = film_category.category_id) AS film_cat) ON film_cat.film_id = film.film_id;
                    
-- 7e. Display the most frequently rented movies in descending order.
SELECT 
    film.title, COUNT(*) AS rented
FROM
    film
        JOIN
    ((SELECT 
    ...
    FROM
        rental
    JOIN inventory ON inventory.inventory_id = rental.inventory_id) AS rented) ON rented.film_id = film.film_id
GROUP BY film.title
...
SELECT 
    name `genres`, SUM(p.amount) `gross revenue`
FROM
    category
        JOIN
    film_category `fc` ON fc.category_id = category.category_id
        JOIN
